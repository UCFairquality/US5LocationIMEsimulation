...
if ~exist(inpFolder, 'dir')
    error('AERMODinputs folder not found. Run Script 1 and 2 first.');
end
...
if ~isfile(recMatFile)
    error('ReceptorMeta.mat not found in %s. Run AutoAERMOD_01_GenReceptors first.', ...
          inpFolder);
end
if ~isfile(inMetaFile)
    error('InputMeta.mat not found in %s. Run AutoAERMOD_02_GenInputs first.', ...
          inpFolder);
end
